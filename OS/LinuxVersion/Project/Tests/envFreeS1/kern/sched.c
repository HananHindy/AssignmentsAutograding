...
fos_scheduler(void)
{
		...
		//else
	{
		...
		if(flag)
		{
			...
			strsplit(l5, WHITESPACE, args, &numOfArgs) ;
...
void sched_kill_all()
{
	...
	{
		flag --;
		struct freeFramesCounters after =calculate_available_frames();

		cprintf("Total available frames = %d\nFree Buffered = %d\nFree Not Buffered = %d\nModified = %d\n",
					after.freeBuffered+ after.freeNotBuffered+ after.modified, after.freeBuffered, after.freeNotBuffered, after.modified);
		if((after.freeBuffered+ after.freeNotBuffered+ after.modified == before.freeBuffered+ before.freeNotBuffered+ before.modified) &&
				(after.freeBuffered == before.freeBuffered) &&
				(after.freeNotBuffered == before.freeNotBuffered) &&
				(after.modified == before.modified))
		{
			cprintf("Congratulations!! test Scenario 1 completed successfully.\n\n");
		}
	}
