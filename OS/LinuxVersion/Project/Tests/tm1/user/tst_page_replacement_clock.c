...
//	cprintf("envID = %d\n",envID);
	...
	//("STEP 0: checking InitialWSError2: INITIAL WS entries ...\n");
	{
		...
		if( ROUNDDOWN(myEnv->pageWorkingSet[8].virtual_address,PAGE_SIZE) !=   0xeebfd000)  panic("InitialWSError2: INITIAL PAGE WS entry checking failed! Review size of the WS..!!");
	...
	{
		if( ROUNDDOWN(myEnv->pageWorkingSet[0].virtual_address,PAGE_SIZE) !=  0xeebfd000)  panic("Page clock algo failed.. trace it by printing WS before and after page fault");
		if( ROUNDDOWN(myEnv->pageWorkingSet[1].virtual_address,PAGE_SIZE) !=  0x80c000)  panic("Page clock algo failed.. trace it by printing WS before and after page fault");
		if( ROUNDDOWN(myEnv->pageWorkingSet[2].virtual_address,PAGE_SIZE) !=  0x803000)  panic("Page clock algo failed.. trace it by printing WS before and after page fault");
		if( ROUNDDOWN(myEnv->pageWorkingSet[3].virtual_address,PAGE_SIZE) !=  0x804000)  panic("Page clock algo failed.. trace it by printing WS before and after page fault");
		if( ROUNDDOWN(myEnv->pageWorkingSet[4].virtual_address,PAGE_SIZE) !=  0x809000)  panic("Page clock algo failed.. trace it by printing WS before and after page fault");
		if( ROUNDDOWN(myEnv->pageWorkingSet[5].virtual_address,PAGE_SIZE) !=  0x80a000)  panic("Page clock algo failed.. trace it by printing WS before and after page fault");
		if( ROUNDDOWN(myEnv->pageWorkingSet[6].virtual_address,PAGE_SIZE) !=  0x80b000)  panic("Page clock algo failed.. trace it by printing WS before and after page fault");
		if( ROUNDDOWN(myEnv->pageWorkingSet[7].virtual_address,PAGE_SIZE) !=  0x800000)  panic("Page clock algo failed.. trace it by printing WS before and after page fault");
		if( ROUNDDOWN(myEnv->pageWorkingSet[8].virtual_address,PAGE_SIZE) !=  0x801000)  panic("Page clock algo failed.. trace it by printing WS before and after page fault");
