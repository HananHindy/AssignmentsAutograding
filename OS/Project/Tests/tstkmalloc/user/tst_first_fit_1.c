...
void _main(void)
{
	...
	//[3] Allocate again [test first fit]
	{
		...
		if ((uint32) ptr_allocations[11] <  (USER_HEAP_START + 8*Mega) || (uint32) ptr_allocations[11] > (USER_HEAP_START + 8*Mega + PAGE_SIZE)) panic("Wrong start address for the allocated space... ");
		//if ((freeFrames - sys_calculate_free_frames()) != 256) panic("Wrong allocation: ");
		if( (sys_pf_calculate_allocated_pages() - usedDiskPages) !=  512) panic("Wrong page file allocation: ");
		if ((freeFrames - sys_calculate_free_frames()) != 0) panic("Wrong allocation: ");
		...
		if ((uint32) ptr_allocations[12] <  (USER_HEAP_START + 14*Mega) || (uint32) ptr_allocations[12] > (USER_HEAP_START + 14*Mega + PAGE_SIZE)) panic("Wrong start address for the allocated space... ");
	...
	//[5] Allocate again [test first fit]
	{
		...
		freeFrames = sys_calculate_free_frames() ;
		usedDiskPages = sys_pf_calculate_allocated_pages();
		ptr_allocations[13] = malloc(1*Mega);
		if ((uint32) ptr_allocations[13] <  (USER_HEAP_START + 1*Mega + 768*kilo) || (uint32) ptr_allocations[13] > (USER_HEAP_START + 1*Mega + 768*kilo + PAGE_SIZE)) panic("Wrong start address for the allocated space... ");
		//if ((freeFrames - sys_calculate_free_frames()) != 512+32) panic("Wrong allocation: ");
		if( (sys_pf_calculate_allocated_pages() - usedDiskPages) !=  256) panic("Wrong page file allocation: ");
		if ((freeFrames - sys_calculate_free_frames()) != 0) panic("Wrong allocation: ");
	}
	cprintf("[AUTO_GR@DING]Congratulations!! test FIRST FIT allocation (1) completed successfully.\n");
