...
//		if( (usedDiskPages - sys_pf_calculate_allocated_pages()) !=  514) panic("Wrong page file free: ");
		...
		if ((uint32) ptr_allocations[8] < (USER_HEAP_START) || (uint32) ptr_allocations[8] > (USER_HEAP_START + PAGE_SIZE)) panic("Wrong start address for the allocated space... ");
		//if ((freeFrames - sys_calculate_free_frames()) != 3*Mega/4096) panic("Wrong allocation: ");
		if( (sys_pf_calculate_allocated_pages() - usedDiskPages) !=  3*Mega/4096) panic("Wrong page file allocation: ");
	}
	...
	//	b) Attempt to allocate large segment with no suitable fragment to fit on
	{
		...
		cprintf("[AUTO_GR@DING]Congratulations!! test FIRST FIT allocation (2) completed successfully.\n");
